...
SOCKET listen_sockv4, listen_sockv6;
...
static HANDLE        serverMainThread;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	g_hInst = hInstance;
	serverMainThread = CreateThread(NULL, 0, ServerMain, NULL, 0, NULL);
	...
	WaitForSingleObject(serverMainThread, INFINITE);
}
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDCANCEL:
			...
			{
				TerminateThread(serverMainThread, 0);
	...
	while (1) {
		...
		if (FD_ISSET(listen_sockv6, &rset)) {
			...
			if (client_sock == INVALID_SOCKET) {
				...
				// 소켓 정보 추가
				AddSocketInfo(client_sock, true, "TEST");
			}
		}
